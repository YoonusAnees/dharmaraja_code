...
// ===================== HELPERS & SUBCOMPONENTS OUTSIDE OF RENDER =====================
...
const PaymentCard = ({ payment, deletingId, onDeletePending }) => {
    ...
    >
      ...
      <div className="flex items-center gap-3 shrink-0 pl-2">
        ...
        {payment.status === "pending" && (
          <button
            onClick={() => onDeletePending(payment._id)}
            disabled={deletingId === payment._id}
            className="p-2 bg-red-500/10 hover:bg-red-500/30 border border-red-500/20 hover:border-red-500/40 rounded-xl text-red-400 hover:text-red-300 transition-all cursor-pointer shadow-md disabled:opacity-50 shrink-0"
...
const PaymentSection = ({ title, icon: Icon, iconColor, items, deletingId, onDeletePending }) => {
...
export default function MemberDashboard() {
  ...
  const handleDeletePending = async (paymentId) => {
    const confirmDelete = window.confirm("Are you sure you want to delete this pending payment?");
